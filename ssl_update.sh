...
if ! "${acme_sh_dir}/acme.sh" --cron --home "${acme_sh_dir}" &> /dev/null; then
    echo "acme.sh cron renewal failed" >&2
    [[ -f "${nginx_systemd_file}" ]] && systemctl start nginx &> /dev/null
    exit 1
fi

if ! "${acme_sh_dir}/acme.sh" --installcert -d "${host}" --fullchainpath "${ssl_chainpath}/xray.crt" --keypath "${ssl_chainpath}/xray.key" --ecc &> /dev/null; then
