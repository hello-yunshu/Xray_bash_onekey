...
Поддерживается развёртывание через Docker. Образ поставляется с предустановленными Xray и Nginx, все функции оригинального скрипта доступны в контейнере. Подробности см. в [Руководстве по развёртыванию Docker](/i18n/languages/ru/DOCKER.md).
...
## Развёртывание AI Skill

Поддерживается автоматическое развёртывание Xray через инструменты ИИ (например, Trae) без ручного взаимодействия. Подробности см. в [Xray_bash_onekey_skill](https://github.com/hello-yunshu/Xray_bash_onekey_skill).

Традиционный подход требует SSH-подключения к серверу, запуска скрипта установки и последовательного ответа на интерактивные вопросы; подход Skill требует лишь сказать ИИ свои требования — он автоматически сгенерирует неинтерактивный скрипт, выполнит его и напрямую вернёт ссылку VLESS.

**Поддерживаемые режимы**: Reality / TLS / ws ONLY / XTLS ONLY

**Использование**: В инструменте ИИ, поддерживающем Skills, просто скажите «Помогите мне настроить Xray на сервере», и ИИ автоматически соберёт информацию, сгенерирует скрипт, выполнит развёртывание и вернёт данные для подключения.
