...
if ! getent group "${cert_group}" >/dev/null 2>&1; then
    echo "Required certificate group not found: ${cert_group}" >&2
    exit 1
fi
cert_user="root"
...
if ! chown -f "${cert_user}:${cert_group}" "${ssl_chainpath}" ||
   ! chown -f "${cert_user}:${cert_group}" "${ssl_chainpath}/xray.crt" "${ssl_chainpath}/xray.key" ||
   ! chmod -f 750 "${ssl_chainpath}" ||
   ! chmod -f 640 "${ssl_chainpath}/xray.crt" "${ssl_chainpath}/xray.key"; then
    echo "Failed to apply layered certificate permissions" >&2
    exit 1
fi

if [[ -f "${xray_systemd_file}" ]] && ! systemctl restart xray >/dev/null 2>&1; then
    echo "Xray restart failed after SSL update" >&2
    exit 1
fi
if [[ -f "${nginx_systemd_file}" ]] && ! systemctl start nginx >/dev/null 2>&1; then
    echo "Nginx start failed after SSL update" >&2
    exit 1
fi
