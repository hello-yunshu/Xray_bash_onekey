...
* входить`idleleo`Откройте меню управления Xray для управления установкой, службами, настройками безопасности и т. д.
...
* Поддерживает сценарии, Xray, Nginx и обновления сертификатов, а также обеспечивает резервное копирование и откат при сбое для критических обновлений.
...
* Реконфигурация обеспечивает три безопасных пути: повторное развертывание с сохранением конфигурации, реконструкция стандартного шаблона и переключение режима.
...
* `idleleo`Именование предыстории:[迷雾后的真容](https://github.com/hello-yunshu/Xray_bash_onekey/wiki/%E8%BF%B7%E9%9B%9C%E5%90%8E%E7%9A%84%E7%9C%9F%E5%AE%B9)
...
|------|------|------|
| Сохранить перераспределение конфигурации | Сохраняйте пользовательские routing/outbounds/DNS и многопользовательские конфигурации, изменяйте только выбранные пользователем поля (порты, пути, параметры UUID, Reality и т. д.). | Изменения структуры передачи (например, ws → gRPC) не поддерживаются. Если вам необходимо изменить комбинацию трансмиссии, воспользуйтесь стандартным шаблоном для ее восстановления. |
...
## Rill Xray AI — помощник по обслуживанию

Встроенный локальный AI-помощник по обслуживанию: в реальном времени следит за здоровьем Xray/Nginx, автоматически диагностирует неисправности и даёт рекомендации — без внешних API. Вход в главном меню — `9` или команда `idleleo --rill-agent`.

**Основные возможности**

* Мониторинг: наблюдение за состоянием служб и конфигурации Xray/Nginx в реальном времени
* Диагностика: определение причины неисправности с указанием уровня уверенности (высокая / средняя / низкая / недостаточно данных)
* Оценка: автоматическое определение типа неисправности и рекомендации; в описании явно указано, разрешена ли автокоррекция или это только рекомендация
* Режимы: интеллектуальная оценка / только наблюдение / безопасное отключение; до включения автокоррекции система не изменяется

**Часто используемые команды**

| Действие | Команда |
|------|------|
| Открыть меню AI-помощника | `idleleo --rill-agent` |
| Установить или восстановить AI-движок | `idleleo --rill-agent-install` |
| Просмотр статуса AI-оценки | `idleleo --rill-agent-status` |
| Запустить AI-диагностику | `idleleo --rill-agent-diagnose` |
| Проверка AI-движка | `idleleo --rill-agent-verify` |
| Безопасно отключить AI-оценку | `idleleo --rill-agent-safe-disable` |
| Удалить Rill AI-движок | `idleleo --rill-agent-uninstall` |

Движок AI-оценки пока находится на стадии тестирования: рекомендуется использовать его как источник диагностических рекомендаций; по умолчанию система не изменяется автоматически.
