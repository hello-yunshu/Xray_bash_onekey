...
fm_MIN_MAIN_VERSION="2.10.0"

if [ -n "$shell_version" ]; then
    oldest=$(printf '%s\n%s\n' "$fm_MIN_MAIN_VERSION" "$shell_version" | sort -V | head -1)
    if [ "$oldest" != "$fm_MIN_MAIN_VERSION" ]; then
        echo "${Error} ${RedBG} file_manager.sh $(gettext "需要主脚本版本") >= ${fm_MIN_MAIN_VERSION}，$(gettext "当前版本"): ${shell_version}，$(gettext "请先更新主脚本") ${Font}"
        return 1
    fi
fi
