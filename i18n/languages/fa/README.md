...
|------|------|------|
| حفظ استقرار مجدد پیکربندی | تنظیمات routing/outbounds/DNS و چند کاربره سفارشی را حفظ کنید، فقط فیلدهای انتخاب شده توسط کاربر (پورت ها، مسیرها، پارامترهای UUID، Reality و غیره) را تغییر دهید. | تغییرات ساختار انتقال (مانند ws → gRPC) پشتیبانی نمی شود. اگر نیاز به تغییر ترکیب انتقال دارید، لطفاً از الگوی استاندارد برای بازسازی آن استفاده کنید. |
...
## Rill Xray AI — دستیار نگهداری

دستیار نگهداری محلی داخلی که بهصورت بلادرنگ وضعیت Xray/Nginx را پایش میکند، خودکار خطاها را تشخیص داده و پیشنهاد میدهد — بدون نیاز به API خارجی. برای ورود در منوی اصلی `9` را وارد کنید یا `idleleo --rill-agent` را اجرا کنید.

**قابلیتهای اصلی**

* پایش: مشاهده بلادرنگ وضعیت سرویسها و پیکربندی Xray/Nginx
* تشخیص: یافتن ریشه خطا همراه با سطح اطمینان (زیاد / متوسط / کم / شواهد ناکافی)
* قضاوت: تشخیص خودکار نوع خطا و ارائه پیشنهاد؛ در شرح بهوضوح مشخص است که اصلاح خودکار مجاز است یا صرفاً پیشنهاد است
* حالتها: قضاوت هوشمند / فقط مشاهده / غیرفعالسازی امن؛ تا زمانی که اصلاح خودکار فعال نشود، سامانه تغییر نمیکند

**دستورات پرکاربرد**

| عملیات | دستور |
|------|------|
| باز کردن منوی دستیار نگهداری | `idleleo --rill-agent` |
| نصب یا تعمیر موتور قضاوت | `idleleo --rill-agent-install` |
| مشاهده وضعیت قضاوت | `idleleo --rill-agent-status` |
| اجرای تشخیص خطا | `idleleo --rill-agent-diagnose` |
| بررسی موتور قضاوت | `idleleo --rill-agent-verify` |
| غیرفعالسازی امن قضاوت | `idleleo --rill-agent-safe-disable` |
| حذف موتور Rill AI | `idleleo --rill-agent-uninstall` |

موتور قضاوت در حال حاضر در مرحله آزمایش است؛ توصیه میشود بهعنوان پیشنهاد تشخیصی استفاده شود و بهصورت پیشفرض سامانه را خودکار تغییر نمیدهد.
